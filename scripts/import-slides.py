...
"""Zet een PowerPoint om in een HTML-deck onder Hoorcollege/.
    ...
    python scripts/import-slides.py "<pad>/Industriële computers en embedded systems - les 1.pptx" --naam Sessie1
...
Net als scripts/import-syllabus.py vertaalt dit script OPMAAK, GEEN WOORDEN.
...
def geweven(alinea, rels):
    ...
    en dat blijft een geldige zin: op de titelslide werd "Tom Demets | Stefan
    Lievens" met "tom.demets@hogent.be | stefan.lievens@hogent.be" een regel
    waarin elke naam naast het adres van de ander kwam te staan.
    """
    stukken = []
    for kind in alinea:
        if kind.tag == f"{A}br":
...
def schrijf_import_md(naam, pptx, gemaakt, totaal):
    ...
    if not bestaand:
        bestaand = ("# Wat de slide-import moest raden\n\n"
                    "Geschreven door `scripts/import-slides.py`. Een sectie per "
...
def main():
    ...
    print(f"{uit.relative_to(REPO)}: {len(secties)} van {len(namen)} slides, "
