...
Reads a Brightspace "Export Components" package and writes one raw HTML file per
content topic into _incoming/, keeping the module tree, the ordering and the
original path in a header comment. Every image the topics reference is copied out
of the zip into img/ (deduplicated against the images already in the repo) and the
src is rewritten to a local relative path, so no /content/enforced/ hotlink ever
reaches a page.
...
This export is full of material that no topic references as a result: a stray
"Een map of bestand verwijderen (rm) 2.html" in the package root, an "embedded/"
tree that is a second, older copy of the Embedded Systems module, and images that
...
name with a numeric id after it, and D2L names a file after whatever the editor
last saved. Only the manifest knows the real title, so the staged name is built
from that.

Stdlib only. Unlike scripts/check-content.py this is an authoring tool: it never
runs in CI or the Stop hook.

    python scripts/import-brightspace.py export.zip
    python scripts/import-brightspace.py export.zip --dry-run
    python scripts/import-brightspace.py export.zip --only "Managed Switch"
"""
...
def load_assignments(zf: zipfile.ZipFile) -> dict[str, tuple[str, str]]:
    ...
    Every labo here ends in a dropbox: the module tree links straight to an
    assignment folder ("Opdracht: partitioneren"), and whatever the student is
    told to do lives in that folder's instructions rather than in a page. Reading
    the manifest alone would silently drop the assignment of every single labo.
    """
...
# Copy. Case-sensitively this export matched 6 of its 16 quicklinks, and the
# 5 dropboxes among the other 10 (virtualiseren, Linux basis, chmod, chown,
...
QUICKLINK_RE = re.compile(r"[?&]rcode=([0-9a-fA-F-]+)", re.I)
...
def rewrite(html: str, topic: Topic, assets: AssetStore,
    ...
    like, so it is chased even without an extension: the Packet Tracer pages
    carry their screenshots as DocumentDownloader URLs on
    chamilo-downloads.hogent.be, the platform this course lived on before
...
def worklist(topics: list[Topic], documents: list[Document], doc_dir: Path) -> str:
    lines = [
        ...
        "orion-convert skill, then add its `reference.js` entry and run",
        "`python scripts/check-content.py`. Delete a row once its page lives under `Labo/<Naam>/`.",
        "",
...
def main() -> int:
    ...
    ap.add_argument("--img-prefix", default="../../../img/",
                    help="what the rewritten src should point at (default: ../../../img/, "
                         "correct for a page in Labo/<Naam>/Theorie/; een "
                         "Opdracht.html zit een niveau hoger)")
    ...
    if unresolved:
        ...
        print("  Those refs were left as-is; check-content.py will flag them.")
    ...
    if not args.dry_run and staged_dirs:
        print(f"  Remember to 'git add {'/ '.join(staged_dirs)}/' -- "
              "check-content.py only accepts assets tracked by git.")
    return 0
