...
"""Genereer het verslagsjabloon (.docx) van een labo uit zijn Opdracht.html.
    ...
    python scripts/export-verslag.py Labo/RS485

Het sjabloon is geen tweede versie van de opdracht die je apart onderhoudt: het
wordt uit de pagina gegenereerd, zodat het niet kan verouderen. Draai dit script
opnieuw in dezelfde commit als een wijziging aan Opdracht.html, en commit de
docx mee. GitHub Pages serveert alleen bestanden die in git zitten.
...
Regel 7 van scripts/check-content.py bewaakt dat het blok afgesloten is en dat
er geen vragen buiten terechtkomen.

Nodig: python-docx (pip install python-docx).
"""
...
def voorblad(doc, titel, intro):
    ...
    het labo over gaat. Regel 8 van scripts/check-content.py eist daarom dat
...
def main():
    ...
    ap.add_argument("-o", "--output", help="pad van de docx (standaard downloads/<Module>-verslag.docx)")
    args = ap.parse_args()
    ...
    naam = args.output or ("downloads/" +
                           "-".join(module.relative_to(repo).parts) + "-verslag.docx")
    uit = repo / naam
